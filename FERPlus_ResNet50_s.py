...
retrain = True
cp_dir = '/home/ubuntu/Notebooks/Models/FER_Resnet50_scratch_best.h5'
log_dir = '/home/ubuntu/Notebooks/Models/FER_Resnet50_scratch_log.csv'


if retrain:    

  resnet = ResNet50(include_top=False, input_shape=(224, 224, 3), classes=8, pooling=None )

  last_layer = resnet.get_layer('conv5_block3_3_bn').output
  x = tensorflow.keras.layers.Flatten(name='flatten')(last_layer)
  out = tensorflow.keras.layers.Dense(8, activation='softmax', name='classifier')(x)
  resnet_scratch = Model(resnet.input, out)
  resnet_scratch.summary()
    
  # compile the model
  opt = tensorflow.keras.optimizers.SGD(learning_rate=0.0001, momentum=0.9)
  resnet_scratch.compile(optimizer= opt, loss='categorical_crossentropy', metrics=['accuracy'])
    
  resnet_scratch.summary()
else:
  best_resnet_scratch = load_model(cp_dir)
  best_resnet_scratch.summary()
...
if retrain:
    ...
    # Fit the model.
    history = resnet_scratch.fit(x=train_iter, 
                  validation_data=(val_iter),
                  epochs=50,
                  class_weight=weights,
                  callbacks=[model_cp, model_log])
    best_resnet_scratch = load_model(cp_dir)  # Retrieve the best model.
...
#load best model
FER = load_model('../Models/FER_Resnet50_scratch_best.h5')
