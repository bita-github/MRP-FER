...
retrain = True
cp_dir ='/home/ubuntu/Notebooks/Models/AffectNet_VGG16.h5'
log_dir = '/home/ubuntu/Notebooks/Models/AffectNet_VGG16_log_.csv'
...
if retrain:    
    ...
    VGGFace16.add(layers.Dropout(0.20))
    VGGFace16.add(layers.Conv2D(filters=256, kernel_size=(3, 3), activation='relu',
    ...
    VGGFace16.add(layers.Dropout(0.50))
    VGGFace16.add(layers.Conv2D(filters=512, kernel_size=(3, 3), activation='relu',
                         padding='same', name='conv5_3'))
    VGGFace16.add(layers.Dropout(0.55))
    ...
    VGGFace16.load_weights('/home/ubuntu/Notebooks/Models/AffectNet_VGG16_v3-1.h5', by_name=True)
    ...
    x = layers.Dense(hidden_dim, activation='relu', name='fc6', kernel_regularizer=l2(5*1e-4))(x)
    x = layers.Dense(hidden_dim, activation='relu', name='fc7', kernel_regularizer=l2(5*1e-4))(x)
...
#     decay_steps = 10
#     decay_rate = 0.5
#     rlrop = InverseTimeDecay(initial_learning_rate, decay_steps, decay_rate)

    opt = tensorflow.keras.optimizers.SGD(learning_rate=1e-3, momentum=0.9)
...
if retrain:
    ...
    train_iter, val_iter = my_generator(generator_args, b_size=64)
    ...
    # define early stopping and learning rate decay
    rlrop = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, verbose=1, cooldown=3)
    stop = EarlyStopping(monitor='loss', patience=5)
    
    # Fit the model. class_weight=weights,
    history = FER_VGGFace16.fit(x=train_iter, 
                  validation_data=(val_iter),
                  epochs=50,
                  callbacks=[model_cp, model_log, rlrop, stop])
    FER = load_model(cp_dir)  # Retrieve the best model.
...
test_iter= test_datagen.flow_from_directory(
        '../data/AffectNet_occ_cropped_grayscale/validation/',
        target_size=(224, 224),
        shuffle = False,
        class_mode=None,
        batch_size=1)
